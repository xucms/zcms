...
class RedisSSID extends Common {
    ...
    public function getid($key) {
        $setRes = $this->redis->get($this->prefix.$key);
        //不存在返回false
        if($setRes === 'false') {
            return false;
        }
    ...
    public function lranges($key,$head,$tail) {
        return $this->redis->lrange($this->prefix.$key,$head,$tail);
    }
    ...
    public function zrank($key,$member,$type = 'desc') {
        ...
        if($type == 'desc') {
            $re = $this->redis->zrevrank($this->prefix.$key,$member);
            //其中有序集成员按score值递减(从大到小)顺序排列，返回其排位
        } else {
    ...
    public function zrangbyscore($key,$star,$end) {
        return $this->redis->ZRANGEBYSCORE($this->prefix.$key,$star,$end);
    }
    ...
    public function zscore($key,$member) {
        return $this->redis->ZSCORE($this->prefix.$key,$member);
    }
    ...
    public function hset_json($redis_key,$token,$id,$data,$timeOut = 0) {
        $redis_table_name = $redis_key['key'].':'.$token;
        //key的名称
        $redis_key_name = $redis_key['field'].':'.$id;
        //field的名称，表示第几个活动
    ...
    public function hget_json($redis_key,$token,$id) {
        $re = $this->redis->hexists($redis_key['key'].':'.$token,$redis_key['field'].':'.$id);
        //返回缓存中该hash类型的field是否存在
        if($re) {
    ...
    public function hget($key,$filed) {
        $re = $this->redis->hget($this->prefix.$key,$filed);
        if(!$re) {
            return false;
        }
        return $re;
    ...
    public function logger($log_content,$position = 'user') {
        ...
        if($position == 'user') {
            ...
            $log_filename = "{$log_dir}/Wap_redis_log.txt";
            //日志名称
        }
